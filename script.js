...
function draw() {
	...
	minutos += segundos ? 0 : 1
	if (minutos == 5) finalizarPartido();
	...
	if (pelota.centro.y < cancha.y + sizePelota / 2) {
		...
		for (var equipo of equipos)
			if (equipo.DT === 'AI') atraerConTodosLosJugadoresDelEquipo(equipo);
	}
	...
	if (!jugadorConPelota) {
		for (var equipo of equipos)
			for (var jugador of equipo.jugadores)
				if (jugador.multiplicadorDeAtraccion > 0 && sq(pelota.centro.x - jugador.centro.x) + sq(pelota.centro.y - jugador.centro.y) < sq(jugador.size)) jugadoresDisputandoPelota.push(jugador);
		if (jugadoresDisputandoPelota.length) {
			...
			for (var equipo of equipos)
				if (equipo === jugadorConPelota.equipo || equipo.DT === 'AI')
					for (jugador of equipo.jugadores) jugador.multiplicadorDeAtraccion = jugador.stats.atraccion / 50;
		}
	}
	if (jugadorConPelota) {
		pelota.centro = {
...
function keyPressed() {
	...
	for (var tecla of teclas)
		for (equipo of equipos)
			if (keyCode === equipo['tecla' + tecla].code) {
...
function keyReleased() {
	...
	for (equipo of equipos) {
		for (var tecla of teclas)
			if (keyCode === equipo['tecla' + tecla].code) equipo['tecla' + tecla].presionada = false;
...
function gol(equipoQueHizoGol) {
	...
	pelota = {
		centro: {
			x: cancha.x + cancha.width / 2 + cos(angulo) * radio,
			y: cancha.y + cancha.height / 2 + sin(angulo) * radio
		},
		velocidad: {
			x: 0,
			y: 0
...
function soltarPelota() {
	...
	pelota.centro = {
		x: jugadorConPelota.centro.x + cos(jugadorConPelota.angulo) * (sizePelota + sizeJugadores) / 2,
		y: jugadorConPelota.centro.y + sin(jugadorConPelota.angulo) * (sizePelota + sizeJugadores) / 2
	};
